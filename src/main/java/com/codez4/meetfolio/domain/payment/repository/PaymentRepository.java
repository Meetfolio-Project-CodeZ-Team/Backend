...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    Optional<Payment> findPaymentByMemberAndPaymentStatusAndCreatedAtIsBetweenOrderByIdDesc(Member member, PaymentStatus paymentStatus, LocalDateTime start, LocalDateTime end);
    ...
    @Query("SELECT p FROM Payment p WHERE p.paymentStatus ='APPROVE' AND DATE_FORMAT(p.createdAt, '%Y-%c') = :month AND p.member.authority ='MEMBER' AND p.member.email != ''")
